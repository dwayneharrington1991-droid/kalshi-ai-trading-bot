    ...
    def _apply_risk_constraints(
        ...
        try:
            ...
            for market_id, fraction in allocation.items():
                ...
                self.logger.warning(
                    f"ALLOCATION DEBUG: market={market_id}, fraction={fraction:.6f}, "
                    f"capital=${self.total_capital:.2f}, dollars=${dollar_allocation:.2f}, "
                    f"minimum=${self.min_position_size:.2f}, opportunity={opp is not None}"
                )
