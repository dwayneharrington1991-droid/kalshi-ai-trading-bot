...
from src.jobs.track import run_tracking, should_exit_position
...
async def test_closed_market_without_result_is_not_locally_settled():
    position = Position(
        market_id="UNVERIFIED", side="YES", entry_price=.4, quantity=1,
        timestamp=datetime.now(), live=True,
    )
    should_exit, reason, _ = await should_exit_position(
        position, 0.0, 0.0, "closed", None
    )
    assert should_exit is False
    assert reason == "resolution_unverified"
...
async def test_run_tracking_closes_position(mock_kalshi_client):
    ...
    try:
        ...
        await run_tracking(db_manager=db_manager, kalshi_client=mock_api, live_mode=True)
...
async def test_non_resolution_exit_requires_successful_sell(
    ...
    mock_api.get_market = AsyncMock(return_value={
        "market": {
            "status": "active",
            "result": None,
                "yes_bid": 65,   # executable bid in cents
                "no_bid": 35,
        }
    })
    mock_api.close = AsyncMock()
...
@patch('src.jobs.execute.place_sell_limit_order', new_callable=AsyncMock)
@patch('src.jobs.track.KalshiClient')
async def test_non_resolution_exit_acceptance_does_not_close_without_fill(
    mock_kalshi_client, mock_place_sell
):
    """
    Order acceptance alone must not close a position; authoritative fill
    projection owns the local quantity and status.
    """
